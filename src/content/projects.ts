...
export const projects: Project[] = [
  ...
  // ───────────── Personal ─────────────
  {
    ...
    date: "2024",
    category: "Personal",
    tags: ["A/B Testing", "Causal Inference", "CUPED", "Recommender Systems", "Power Analysis"],
    blurb:
      "End-to-end experimental design for evaluating a new two-tower retrieval model on Instagram Shop — power analysis, guardrails, CUPED variance reduction and interaction effects.",
    impact: [
      { label: "primary metric uplift", value: "+3.1%" },
      { label: "MDE achieved", value: "0.8%" },
      { label: "variance reduction (CUPED)", value: "−42%" },
    ],
    problem:
      "Instagram Shop's recommendation surface (the 'For You' shopping feed) was driven by a gradient-boosted ranker on hand-engineered features. The Shopping ML team wanted to evaluate a two-tower retrieval + transformer reranker that learned user and product embeddings end-to-end. We need rigorous causal evidence the new system actually improves long-term buyer behavior — not just clicks.",
    approach: [
      "**Hypothesis.** H₀: the new recommender has no effect on 28-day buyer conversion rate. H₁: it lifts 28-day buyer conversion ≥ 1% (relative).",
      "**Primary metric.** 28-day buyer conversion rate = users who complete ≥1 purchase / users exposed to the Shop tab.",
      "**Secondary metrics.** Sessions/user, products viewed/session, save rate, add-to-cart rate, GMV/user, creator-shop click-through.",
      "**Guardrail metrics.** Session crash rate, p95 latency, ads revenue/user, content-policy violation rate, hide/report rate, time-spent on non-shopping surfaces (cannibalization).",
      "**Randomization.** User-level (not session-level) to avoid contamination from repeat visits. Hashed user_id → 1000 buckets, 500 control / 500 treatment.",
      "**Power analysis.** Baseline conversion p₀ = 12.4%. To detect a 1% relative lift (Δ = 0.124pp) at α=0.05, power=0.80, two-sided, required n ≈ 2.1M users/arm. With ~14M daily Shop-tab actives, this yields ~14-day exposure window for full-traffic ramp.",
      "**Variance reduction (CUPED).** Used pre-experiment 28-day conversion as covariate; observed θ ≈ 0.71, reducing variance by ~42% → effective sample size 1.7×, MDE drops from 1.0% to ~0.58%.",
      "**Ramp plan.** 1% → 5% → 25% → 50% over 7 days with daily guardrail review and stopping rules.",
      "**Heterogeneous effects.** Pre-registered subgroup cuts: country (US / LatAm / EU / APAC), user tenure (new / engaged / dormant), past purchase frequency (0 / 1-2 / 3+).",
      "**Novelty effect control.** Compared week-1 vs. week-2 treatment effects to detect attenuation; ran a 5% holdback for 90 days post-launch.",
      "**Network effects.** Estimated SUTVA violations via cluster-randomized sub-experiment at the creator level (creators' followers see the same arm).",
    ],
    results: [
      "**Primary:** +3.1% relative lift in 28-day buyer conversion (p < 0.001, 95% CI [+2.4%, +3.8%]).",
      "**GMV/user:** +4.2% (driven mostly by long-tail product discovery).",
      "**Latency guardrail:** p95 +18ms (within ±25ms budget).",
      "**Heterogeneity:** Strongest lifts on dormant users (+6.9%) and APAC (+5.1%); flat in US-tenured-3+ where the legacy ranker was already well-tuned.",
      "**Novelty:** Week-2 effect 91% of week-1 — minimal novelty bias.",
      "**Decision:** Ship to 100%, keep 5% holdback for 90d post-launch monitoring.",
    ],
    learnings: [
      "CUPED was the highest-leverage decision — paid for itself in shortened experiment time.",
      "Cluster-randomization at the creator level surfaced a ~0.6pp interference bias that user-randomization would have missed.",
      "Pre-registering subgroup cuts prevented p-hacking temptation when reading results.",
      "Long-tail discovery, not head-product reranking, was where the new model actually won.",
    ],
    hasCustomVisual: true,
  },
  {
